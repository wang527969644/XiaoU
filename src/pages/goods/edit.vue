...
export default {
  ...
  methods: {
    ...
    //根据ID找到当前点击的商品详情
    async getGoodsById() {
      const res = await getGoodsById(this.formData.id);
      if (res) {
        this.formData = res;
        // 获取二级分类
        this.firstChange(this.formData.first_cateid);
        // 获取规格列表
        this.goodsChange(this.formData.specsid, false);
        // 更新商品图片
        this.goodsImgList = [{ url: this.categoryImg + this.formData.img }];
        // 手动设置富文本编辑器的内容
